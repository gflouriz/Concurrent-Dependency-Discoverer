/* Name: Georgios Flouri
   GUID: 2382904F
   Assignment Title: SP Exercise 2
...
struct Table {
	private:
		...
		std::mutex m;
		std::unordered_map<std::string, std::list<std::string>> myTable; 

	public:
	
	void insert(std::string name, std::list<std::string> fileExt){
		std::unique_lock<std::mutex> lock(m);
		myTable.insert({name,fileExt});
	}
	
	auto find(std::string toFind) {
		std::unique_lock<std::mutex> lock(m);
		return myTable.find(toFind);
	}
...
struct Queue {
	...
	auto pop_front(){
		std::unique_lock<std::mutex> lock(m);
		std::string queueFront = myQueue.front();
		myQueue.pop_front();
		return queueFront;
	}
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    theTable.insert( name, {} ); // Removed extra {} causing error of 1 argument given rather than 2.
    // ... append file name to workQ
...
void threadExe() { 
	while(workQ.size()>0) { // for each file on the workQ
		
		std::string filename = workQ.pop_front();
		
		if(theTable.find(filename) != theTable.end()) { // lookup dependencies and invoke 'process'
...
void threadNum() {
	...
	if (crawlerThreads == NULL){
		...
		threads = std::stoi(crawlerThreads);
	}
}

void threadInit(int threads){ 
	for (int i=0;i<threads;i++){ // Creates threads
		threadList.emplace_back([](){threadExe();});
	}
	
	for (int i=0; i<threadList.size();i++){ // Joins threads
		threadList[i].join();
	}
}
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    theTable.insert( obj, { argv[i] } ); // Again removed extra {} for the insert() calls.
  ...
  //4. (see functions declared outside main)
  threadInit(threads);
